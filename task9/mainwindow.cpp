...
{
    ui->tb_result->append("Свободное место на сервере, байт: " + QString::number(freeSpace));
}
...
void MainWindow::DisplaySuccess(uint16_t typeMess)
{
    switch (typeMess) {
    case CLEAR_DATA:{
        ui->tb_result->append("Память на сервере очищена");
        break;
    }
    default:
        break;
...
void MainWindow::on_pb_request_clicked()
{
   ...
   switch (ui->cb_request->currentIndex()){
    ...
    if(header.idData == SET_DATA){
        ...
        if(header.len == 0){
            ui->tb_result->append("Ошибка: нельзя передать пустую строку");
        }
        else{
